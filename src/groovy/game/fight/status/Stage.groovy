        ...
        if ((addToStage > 0 || selfStageAction) && effectSucces && !openentStageAction)
        {
            ...
            if (attackFightPlayer["${statName}Stage"] + addToStage > 5 || attackFightPlayer["${statName}Stage"] + addToStage < -5)
            {
                ...
                if (attackFightPlayer["${statName}Stage"] + addToStage == 6)
                {
                    ...
                    if (!selfStageAction)
                        fight.roundResult.battleActions.add(new MessageLog("${attackFightPlayer.ownerPokemon.pokemon.name}`s ${statName} went up."))
                ...
                {
                    if (!selfStageAction)
                        fight.roundResult.battleActions.add(new MessageLog("${attackFightPlayer.ownerPokemon.pokemon.name}`s ${statName} can`t go up anymore."))
                }
            }
            else
            {
            ...
            if (defendingFightPlayer["${statName}Stage"] + addToStage < -5 || openentStageAction)
            {
                ...
                if (addToStage > 0) // positieve actie op tegenstander
                {
                    if ((defendingFightPlayer["${statName}Stage"] + addToStage) <= 6)
                    {
                        ...
                        fight.roundResult.battleActions.add(new MessageLog("${defendingFightPlayer.ownerPokemon.pokemon.name}`s ${statName} went up."))
                ...
                {
                    defendingFightPlayer["${statName}Stage"] = -6;
                    fight.roundResult.battleActions.add(new MessageLog("${defendingFightPlayer.ownerPokemon.pokemon.name}`s ${statName} lowers."))

                }
                else
                {
                    fight.roundResult.battleActions.add(new MessageLog("${defendingFightPlayer.ownerPokemon.pokemon.name}`s ${statName} can`t lower anymore."))
